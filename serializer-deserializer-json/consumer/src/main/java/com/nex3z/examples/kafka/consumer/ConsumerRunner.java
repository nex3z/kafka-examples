...
public class ConsumerRunner implements ApplicationRunner {
    ...
    @Autowired
    private Consumer<String, CounterRecord> consumer;
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        try {
            ...
            while (true) {
                final ConsumerRecords<String, CounterRecord> records = consumer.poll(Duration.ofSeconds(1));

                if (records.count() == 0) {
                    continue;
                }

                for (ConsumerRecord<String, CounterRecord> record : records) {
